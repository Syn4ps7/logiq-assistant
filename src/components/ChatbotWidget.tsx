...
export function ChatbotWidget() {
  ...
  return (
    <div
      id="logiq-chatbot"
      className={`logiq-chatbot--widget fixed bottom-6 right-6 z-50 ${
        isActive ? "logiq-chatbot--active" : ""
      } ${isProactive ? "logiq-chatbot--proactive" : ""}`}
    >
      {/* Chat panel */}
      {isActive && (
        <div className="mb-4 w-80 sm:w-96 bg-card rounded-xl shadow-2xl border overflow-hidden animate-fade-in-up">
          <div className="bg-primary text-primary-foreground p-4 flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-sm">LogIQ Assistant</h3>
          ...
          <div className="h-72 p-4 overflow-y-auto bg-muted/30">
            <div className="bg-muted rounded-lg p-3 text-sm max-w-[80%]">
              <p className="text-foreground">
                Bonjour — Je peux vérifier la disponibilité, estimer un prix, ou vous expliquer nos Conditions Générales. Comment puis-je vous aider ?
              </p>
            </div>
          </div>
          ...
          <div className="p-3 border-t flex gap-2">
            <input
              ...
              disabled
            />
            <Button variant="petrol" size="icon" disabled aria-label="Envoyer">
          ...
          <div className="px-3 pb-2">
            <p className="text-[10px] text-muted-foreground text-center">
              Chatbot en cours d'intégration — Disponible prochainement
            </p>
          </div>
        </div>
      )}
